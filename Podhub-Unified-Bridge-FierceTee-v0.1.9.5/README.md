# FierceTee Unified Bridge v0.1.9.5

Bản riêng này chỉ chấp nhận license thuộc team `Fiercetee` có routing tới
`https://api.fiercetee.com`. Team API Key không được nhúng vào extension;
extension chỉ nhận token ngắn hạn sau khi kích hoạt key nhân viên.

Mã nguồn trong thư mục gốc được giữ đầy đủ để tiếp tục bảo trì. Chỉ phân phối
file ZIP tạo bởi `npm run build`, vì JavaScript trong bản release đã obfuscate.

## Nguồn gốc

Bản này được tách từ Podhub Unified Bridge `0.1.9.5-stable-queue-sync`, không
phải từ dòng Podhub GPT Bridge `1.1.x`.

# Podhub GPTs Bridge

## Stable Queue Sync — v0.1.9.5
...
- Marketplace dùng endpoint Unified ổn định để server xử lý đồng thời Spy, license entitlement và tạo job.
