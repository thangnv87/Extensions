...
(function () {
  ...
  async function refreshTeamRouting() {
    ...
    const routing = normalizeTeamRouting(payload.data?.routing || payload.data);
  ...
  async function activateLicense(licenseKey) {
    ...
    await chromeStorageSet({
      ...
      [TEAM_ROUTING_STORAGE_KEY]: normalizeTeamRouting(payload.data.routing)
    });
    const routing = normalizeTeamRouting(payload.data.routing);
    await ensureOriginPermission(routing.api_base_url);
    return payload.data;
  }
...
marketplace_listings chỉ chứa các key: ${markets.join(', ')}. Không markdown, không giải thích.`;
  ...
  function buildPanel() {
    ...
    function ensureLauncher() {
      ...
      launcherBtn.title = 'Podhub GPT Bridge';
    ...
    hdr.innerHTML = `
      <div>
        <div style="font-size:14px;font-weight:900;color:#e2e8f0">Podhub GPTs v${VERSION}</div>
