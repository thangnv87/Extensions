...
async function api(path, options = {}) {
  ...
  const response = await fetch(TOOLS_ORIGIN + path, {
    ...
    headers: {
      ...
      'X-FierceTee-Request-Id': options.requestId || `ext_${crypto.randomUUID()}`,
      ...(options.headers || {})
    }
  });
...
async function activate(licenseKey) {
  ...
  if (!normalizedKey) throw new Error('Vui lòng nhập license key.');
  ...
  const body = {
    ...
    installation_id: id,
    bridge_gateway: true,
    app_name: 'FierceTee GPTs Bridge',
  ...
  try {
    ...
    try {
      ...
      payload = await apiPost('/api/extension-keys/activate', body);
    }
  }
...
async function refreshConfig() {
  ...
  try {
    config = await api('/api/extension/config?bridge_gateway=1', {
      headers: {'X-FierceTee-Bridge-Client': 'bridge_api_v1'}
    });
  } catch {
...
async function fetchAsset(message) {
  ...
  if (source.data_url) return {data_url: source.data_url, name: source.name || source.filename || 'design.png', type: source.type || 'image/png'};
  let directUrl = source.url || source.image_url || source.asset_url || source.thumbnail_url || source.sourceImageUrl;
  let url = directUrl;
  ...
  const saved = await storageGet([STORAGE.token]);
  const targetUrl = authenticated ? (url.startsWith('http') ? url : TOOLS_ORIGIN + url) : url;
  ...
  if (!response.ok) {
    if (authenticated && directUrl && /^https?:\/\//i.test(directUrl)) {
      const fallbackResp = await fetch(directUrl);
      if (fallbackResp.ok) {
        const blob = await fallbackResp.blob();
        return {
          data_url: await blobToDataUrl(blob),
          name: source.name || source.filename || 'design.png',
          type: fallbackResp.headers.get('Content-Type') || blob.type || 'image/png'
        };
      }
    }
  ...
  return {
    data_url: await blobToDataUrl(blob),
    name: source.name || source.filename || decodeURIComponent(response.headers.get('X-FierceTee-Design-Name') || '') || 'design.png',
...
async function uploadResult(message) {
  ...
  return api(path, {method: 'POST', body: blob, headers: {'Content-Type': blob.type || 'image/png', 'X-FierceTee-Result-Kind': kind}});
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  const run = async () => {
    ...
    if (message?.type === 'PUB_GET_ACTIVE_RUN' || message?.type === 'PUB_GET_ACTIVE_JOB') {
      const active = (await storageGet([STORAGE.activeRun]))[STORAGE.activeRun] || null;
      return active ? { ...active, job: active.job || active } : null;
    }
    if (message?.type === 'PUB_NEXT_JOB' || message?.type === 'PUB_FETCH_NEXT_JOB') return fetchNextJob(await getModule(message.moduleId));
    ...
    if (message?.type === 'PUB_DOWNLOAD_FILE') {
      const id = await chrome.downloads.download({
        url: message.url, filename: message.filename || 'design.png', saveAs: Boolean(message.saveAs)
      });
      return {downloadId: id};
    }
    throw new Error(`UNKNOWN_MESSAGE: ${message?.type}`);
  };
