# Podhub GPTs Bridge EX

## Stable Queue Sync — v0.1.9.5 EX Clear

This variant uses `https://ex.podhub.space` for activation, configuration, job queues, asset downloads, and result uploads. It contains readable source code and no obfuscated runtime files.
...
- Marketplace dùng endpoint Unified ổn định trên `https://ex.podhub.space` để server xử lý đồng thời Spy, license entitlement và tạo job.
...
- Extension chỉ gọi gateway `https://ex.podhub.space/api/bridge/v1` bằng access token của license.
...
The extension expects GPT links and optional module endpoint overrides from `https://ex.podhub.space`.
