...
const outputDir = path.resolve(__dirname, '../Podhub-GPTs-Bridge-EX-Obfuscated');

test('bản EX mã hóa kích hoạt đúng server và lưu access token', async () => {
  const saved = {};
  const calls = [];
  let listener;
  const workerScope = {
    ...
    fetch: async (url, options = {}) => {
      ...
      if (url.endsWith('/api/extension/activate')) {
        return new Response(JSON.stringify({
          success: true,
          data: {access_token: 'ex-test-token', user: {username: 'ex-canary'}}
  ...
  assert.equal(saved.pub_license_token, 'ex-test-token');
  assert.equal(calls[0].url, 'https://ex.podhub.space/api/extension/activate');
  assert.equal(calls[0].options.headers.Authorization, undefined);
  assert.equal(calls.some(call => call.url.startsWith('https://tools.podhub.space')), false);
